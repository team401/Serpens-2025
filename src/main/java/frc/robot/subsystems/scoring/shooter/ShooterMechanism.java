...
public class ShooterMechanism {
  ...
  private ShooterSpeeds calculatePoseBasedSpeeds() {
    ...
    // The direction of the shot to take. This will be 90 degrees to the left of the robot's
    // direction.
    final Rotation2d shotDirectionAngle = robotPose.getRotation().plus(Rotation2d.kCCW_90deg);
    ...
    */
    ...
    Translation2d rayDirectionVector =
        new Translation2d(shotDirectionAngle.getCos(), shotDirectionAngle.getSin()); // d
    ...
    double denominator = GeomUtil.cross(segmentVector, rayDirectionVector);
    ...
    if (Math.abs(denominator) < 1e-9) {
      ...
      // First check that the shot is in the forward shot direction
      double u = GeomUtil.cross(r, segmentVector) / denominator; // u = (r x s) / (s x d)

      // If u is negative, the intersection is backward along the shotDirection vector and the shot
      // would be in the wrong direction
      if (u < 0.0) {
        isShotAttainable = false;

        // Log a sentinel value to record that this case was reached
        Logger.recordOutput("scoring/shooter/clampedShotDistance", -1.0);

        // Give up on making the shot until the robot turns within 90 degrees of the goal
        return ZERO_SPEEDS;
      }

      double t = GeomUtil.cross(r, rayDirectionVector) / denominator; // t = (r x d) / (s x d)
    ...
    double leftSpeedRPM = JsonConstants.shooterConstants.distanceMetersToLeftRPM.get(shotDistance);
    double rightSpeedRPM =
        JsonConstants.shooterConstants.distanceMetersToRightRPM.get(shotDistance);

    AngularVelocity leftSpeed = RPM.of(leftSpeedRPM);
    AngularVelocity rightSpeed = RPM.of(rightSpeedRPM);
